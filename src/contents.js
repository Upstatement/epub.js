...
class Contents {
  ...
  /**
		...
		*/
  viewport(options) {
    let _width; let _height; let _scale; let _minimum; let _maximum; let _scalable;
  ...
  addStylesheetRules(rules, key) {
    ...
    if (Object.prototype.toString.call(rules) === '[object Array]') {
      for (let i = 0, rl = rules.length; i < rl; i++) {
        let j = 1; let rule = rules[i]; const selector = rules[i][0]; let propStr = '';
      ...
      selectors.forEach(selector => {
        ...
        if (Array.isArray(definition)) {
          definition.forEach(item => {
            ...
            const result = _rules.map(rule => `${rule}:${item[rule]}`).join(';');
            styleSheet.insertRule(`${selector}{${result}}`, styleSheet.cssRules.length);
          });
        } else {
          const _rules = Object.keys(definition);
          const result = _rules.map(rule => `${rule}:${definition[rule]}`).join(';');
          styleSheet.insertRule(`${selector}{${result}}`, styleSheet.cssRules.length);
        }
      });
    }
