    ...
    const search = function(nodeList){
      ...
      if (pos != -1){
        ...
        if (pos < nodeList[startNodeIndex].length){
          ...
          const beforeEndLengthCount =  nodeList.slice(0, endNodeIndex).reduce((acc, current) => acc + current.textContent.length, 0) ;
          range.setEnd(endNode, beforeEndLengthCount > endPos ? endPos : endPos - beforeEndLengthCount );
          cfi = section.cfiFromRange(range);

          let excerpt = nodeList.slice(0, endNodeIndex + 1).reduce((acc, current) => acc + current.textContent, '');
